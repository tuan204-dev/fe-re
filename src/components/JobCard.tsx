                ...
                <div className="flex-grow">
                    <div className="flex justify-between items-start">
                        ...
                        <div className="flex gap-2">
                            {job.recruitingStatus !== RecruitingStatus.CLOSED && <button className="text-gray-400 hover:text-blue-600 p-2 rounded-full hover:bg-blue-50">
                                <FaEdit />
                            </button>}
                            {[RecruitingStatus.DRAFT, RecruitingStatus.PAUSED].includes(job?.recruitingStatus ?? 0) && <button onClick={handlePublicJob} className="text-gray-400 hover:text-blue-600 p-2 rounded-full hover:bg-blue-50 text-xl">
                                <MdPublish />
                            </button>}
                            {job?.recruitingStatus === RecruitingStatus.PUBLIC && <button onClick={handlePauseJob} className="text-gray-400 hover:text-yellow-600 p-2 rounded-full hover:bg-yellow-50">
                                <FaPause />
                            </button>}
                            {job.recruitingStatus !== RecruitingStatus.CLOSED && <button onClick={handleCloseJob} className="text-gray-400 hover:text-red-600 p-2 rounded-full hover:bg-red-50">
                                <FaTrash />
                            </button>}
                        </div>
                    </div>
                    <div className="flex flex-wrap gap-4 mt-4">
                        <div className="flex items-center text-gray-600">
                            ...
                            <span>${formatNumber(job.salaryRange.min)} - ${formatNumber(job.salaryRange.max)}/month</span>
                        </div>
                        {/* <div className="flex items-center text-gray-600">
